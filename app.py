...
# TTLCache = Time To Live Cache (expires after N seconds)
_cache = TTLCache(maxsize=100, ttl=300)  # 5 minute cache
...
def create_app(config_name=None):
    ...
    def add_cache_headers(response):
        ...
        CSS/JS/Images are cached for 1 year.
        HTML pages are not cached (always fresh).
        """
        if request.path.startswith('/static/'):
            # Cache static files for 1 year
            response.headers['Cache-Control'] = \
                'public, max-age=31536000, immutable'
        elif request.path in ['/', '/categories', '/about']:
            # Cache HTML pages for 5 minutes
            response.headers['Cache-Control'] = \
                'public, max-age=300, must-revalidate'
        else:
            # Don't cache API or admin responses
            response.headers['Cache-Control'] = \
